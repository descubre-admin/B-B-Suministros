...
type Item = { product:Product; qty:number; metersPerUnit:number|null; unitPrice:number; basePrice:number; supplierDiscount:number; profitPercent:number }
type DraftQuoteItem = { productId:string; name:string; brand:string|null; sku:string|null; qty:number; unitPrice:number; basePrice?:number; supplierDiscount?:number; profitPercent?:number; metersPerUnit?:number|null }
...
export default function NewQuotePage() {
    ...
    try {
      ...
      if(edit?.quoteId && Array.isArray(edit.items)){
        ...
        setItems(edit.items.map(d=>{
          ...
          return { product, qty:Number(d.qty)||1, unitPrice:Number(d.unitPrice)||0, basePrice:Number(d.basePrice??product.listPrice??product.cost??d.unitPrice)||0, supplierDiscount:Number(d.supplierDiscount??product.supplierDiscount??0)||0, profitPercent:Number(d.profitPercent??product.markup_percent??0)||0, metersPerUnit:d.metersPerUnit == null ? null : Number(d.metersPerUnit) }
        }))
  ...
  const filtered = products
    ...
    .slice(0,12)
    .map(x=>x.p)
  const billedUnits=(i:Item)=>i.qty*(i.metersPerUnit&&i.metersPerUnit>0?i.metersPerUnit:1)
  const subtotal = useMemo(()=>items.reduce((s,i)=>s+billedUnits(i)*i.unitPrice,0),[items])
  const total = Math.max(0, subtotal - discount)
  const totalCost = useMemo(()=>items.reduce((s,i)=>s + billedUnits(i) * (i.basePrice * (1-i.supplierDiscount/100)),0),[items])
  ...
  function add(p:Product){ setItems(prev=>{ const ix=prev.findIndex(i=>i.product.id===p.id); if(ix>=0)return prev.map((i,n)=>n===ix?{...i,qty:i.qty+1}:i); const base=Number(p.listPrice??p.cost??p.sale_price); const disc=Number(p.supplierDiscount??0); const profit=Number(p.markup_percent??0); const calculated=calcSale(base,disc,profit); return [...prev,{product:p,qty:1,metersPerUnit:null,basePrice:base,supplierDiscount:disc,profitPercent:profit,unitPrice:calculated>0?calculated:Number(p.sale_price)}] }); setQuery('') }
  ...
  function patchSalePrice(idx:number, salePrice:number){setItems(v=>v.map((x,n)=>{if(n!==idx)return x;const cost=Number(x.basePrice||0)*(1-Number(x.supplierDiscount||0)/100);const profitPercent=cost>0?((salePrice-cost)/cost)*100:0;return {...x,unitPrice:Math.max(0,salePrice),profitPercent:Number(profitPercent.toFixed(2))}}))}
  ...
  async function saveQuote(showAlert=true):Promise<SavedQuote|null>{
    ...
    const rows = items.map(i=>({ quote_id:q!.id, product_id:i.product.id, description:i.product.name, quantity:i.qty, unit_price:i.unitPrice, line_total:billedUnits(i)*i.unitPrice, base_price:i.basePrice, supplier_discount_percent:i.supplierDiscount, profit_percent:i.profitPercent, meters_per_unit:i.metersPerUnit, total_meters:i.metersPerUnit ? i.qty*i.metersPerUnit : null }))
  ...
  async function pdf(){
    ...
    items.forEach(i=>{const meterText=i.metersPerUnit?` · ${i.metersPerUnit} m/u · ${i.qty*i.metersPerUnit} m total`:'';const desc=`${i.product.name}${i.product.brand?` · ${i.product.brand}`:''}${meterText}`;doc.text(String(i.qty),14,y);doc.text(desc.slice(0,58),30,y);doc.text(money(i.unitPrice),156,y,{align:'right'});doc.text(money(billedUnits(i)*i.unitPrice),196,y,{align:'right'});y+=8;if(y>270){doc.addPage();y=20}})
  ...
  function whatsapp(){
    ...
    const detail=items.map(i=>`${i.qty} x ${i.product.name}${i.metersPerUnit?` (${i.metersPerUnit} m/u = ${i.qty*i.metersPerUnit} m)`:''}: ${money(billedUnits(i)*i.unitPrice)}`).join('\n')
  ...
  return <AppShell>
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_360px]">
      <section className="space-y-4">
        ...
        <div className="rounded-2xl border bg-white p-4"><div className="flex items-center justify-between"><div className="font-semibold">Vista previa del presupuesto</div><div className="text-xs text-slate-500">Se actualiza en vivo</div></div><div className="mt-4 rounded-xl border bg-white p-5"><div className="flex items-start justify-between border-b pb-4"><div><img src="/logo-bb-horizontal.png" alt="B&B Suministros" className="h-14 w-auto"/><div className="mt-1 text-xs text-slate-500">Electricidad · Iluminación · Ferretería</div></div><div className="text-right"><div className="text-lg font-bold">PRESUPUESTO</div><div className="text-sm font-semibold">N° {savedQuote?String(savedQuote.number).padStart(6,'0'):'------'}</div><div className="text-xs text-slate-500">{new Date().toLocaleDateString('es-AR')}</div></div></div><div className="py-4"><div className="text-xs font-semibold uppercase tracking-wide text-slate-500">Cliente</div><div className="mt-1 text-lg font-bold">{displayCustomerName}</div>{customer&&<div className="mt-0.5 text-sm text-slate-500">Cliente N° {String(customer.customer_number).padStart(5,'0')}{customer.city?` · ${customer.city}`:''}</div>}</div><div className="overflow-x-auto"><div className="min-w-[900px]"><div className="grid grid-cols-[1fr_100px_85px_100px_85px_110px_70px_90px_90px_105px_36px] gap-2 border-b pb-2 text-[11px] font-semibold uppercase text-slate-500"><span>Producto</span><span>Precio lista</span><span>Desc. prov.</span><span>Costo</span><span>Ganancia</span><span>Venta</span><span>Cant.</span><span>Metros/u</span><span>Total m</span><span className="text-right">Importe</span><span></span></div><div className="divide-y">{items.map((i,idx)=>{const cost=i.basePrice*(1-i.supplierDiscount/100);return <div key={`${i.product.id}-${idx}`} className="grid grid-cols-[1fr_100px_85px_100px_85px_110px_70px_90px_90px_105px_36px] items-center gap-2 py-3"><div className="min-w-0"><div className="font-medium">{i.product.name}</div><div className="text-xs text-slate-500">{[i.product.brand,i.product.sku].filter(Boolean).join(' · ')}</div></div><input type="number" min="0" step="0.01" value={i.basePrice} onFocus={clearZeroOnFocus} onChange={e=>patchPricing(idx,{basePrice:Number(e.target.value)})} className="rounded-lg border px-2 py-2 text-sm"/><div className="relative"><input type="number" min="0" max="100" step="0.1" value={i.supplierDiscount} onFocus={clearZeroOnFocus} onChange={e=>patchPricing(idx,{supplierDiscount:Number(e.target.value)})} className="w-full rounded-lg border px-2 py-2 pr-6 text-sm"/><span className="absolute right-2 top-2 text-sm text-slate-400">%</span></div><div className="text-sm font-medium">{money(cost)}</div><div className="relative"><input type="number" min="0" step="0.1" value={i.profitPercent} onFocus={clearZeroOnFocus} onChange={e=>patchPricing(idx,{profitPercent:Number(e.target.value)})} className="w-full rounded-lg border px-2 py-2 pr-6 text-sm"/><span className="absolute right-2 top-2 text-sm text-slate-400">%</span></div><input type="number" min="0" step="0.01" value={Number(i.unitPrice.toFixed(2))} onFocus={clearZeroOnFocus} onChange={e=>patchSalePrice(idx,Number(e.target.value))} className="rounded-lg border px-2 py-2 text-sm font-semibold"/><input type="number" min="0.01" step="0.01" value={i.qty} onFocus={clearZeroOnFocus} onChange={e=>patch(idx,{qty:Number(e.target.value)})} className="rounded-lg border px-2 py-2 text-sm"/><input type="number" min="0" step="0.01" value={i.metersPerUnit ?? ''} placeholder="Ej: 100" onChange={e=>patch(idx,{metersPerUnit:e.target.value===''?null:Number(e.target.value)})} className="rounded-lg border px-2 py-2 text-sm"/><div className="text-sm font-semibold">{i.metersPerUnit ? `${(i.qty*i.metersPerUnit).toLocaleString('es-AR')} m` : '—'}</div><div className="text-right font-semibold">{money(billedUnits(i)*i.unitPrice)}</div><button onClick={()=>setItems(v=>v.filter((_,n)=>n!==idx))} className="rounded-lg p-2 text-slate-400 hover:bg-red-50 hover:text-red-600"><Trash2 size={16}/></button></div>})}</div>{!items.length&&<p className="py-6 text-center text-sm text-slate-500">Agregá productos desde el buscador.</p>}</div></div><div className="mt-4 ml-auto max-w-xs border-t pt-3 text-sm"><div className="flex justify-between"><span>Subtotal</span><b>{money(subtotal)}</b></div>{discount>0&&<div className="mt-1 flex justify-between"><span>Descuento cliente</span><span>-{money(discount)}</span></div>}<div className="mt-2 flex justify-between text-lg font-bold"><span>TOTAL</span><span>{money(total)}</span></div><div className="mt-1 text-xs text-slate-500">Vigencia: {validity} hs.</div></div></div><p className="mt-3 text-xs text-slate-500">Precio lista, descuento de proveedor, costo y ganancia son datos internos. El PDF del cliente muestra producto, cantidad, metros cuando corresponda, precio de venta e importe. Dejá “Metros/u” vacío para productos que se venden por unidad.</p></div>
      </section>
